...
public class GerenciadorLogBook {
	...
	public void adicionarLogBook(LogBook logbook)
		...
		if (logbook.getCodLogBook() == null || logbook.getAssunto() == null
				|| logbook.getHora() == null || logbook.getData() == null
				|| logbook.getProfessor() == null || logbook.getTurma() == null
				|| logbook.getDisciplina() == null
				|| logbook.getListaPergunta() == null
				|| logbook.getListaPratica() == null
				|| logbook.getListaResposta() == null)
			throw new LogBookSemDadosException(
					"Imposs�vel adicionar LogBook sem dados!");
		try {
			pesquisarLogBook(logbook.getCodLogBook());
			throw new LogBookJaCadastradoException(
					"J� existe um LogBook cadastrado com o c�digo informado!");
		} catch (LogBookInexistenteException e1) {
			listaLogBooks.add(logbook);
		}
	}
	public LogBook editarLogBook(LogBook logbook)
			throws LogBookInexistenteException {
	...
	public LogBook pesquisarLogBook(String codigoLogBook)
		...
		throw new LogBookInexistenteException(
				"N�o existe este LogBook com este c�digo no Sitema LogBook");
	}
